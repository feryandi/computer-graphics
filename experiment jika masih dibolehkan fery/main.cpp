...
void createEyeBall(Polygon *p) {
	(*p).addPoint(24, 48, 0, 0);
	(*p).addPoint(0, 48, 0, 0);
	(*p).addPoint(0, 0, 0, 0);
	(*p).addPoint(48, 0, 0, 0);
	(*p).addPoint(48, 48, 0, 0);
	(*p).addPoint(24, 48, 0, 3);
}
...
void doImut(FrameBuffer *FB, vector<Polygon*> *polies, Polygon *mouth, Polygon *left_eye, Polygon *left_eyebrow, Polygon *left_eyeball, int r) {
	// Imut
	int f = 0;
	while ( f < 15 ) {
		(*FB).canvas();
	...
	while ( f < 5 ) {
		...
		//(*left_eyeball).movePoint(1, 0, 2*r, 0);
		(*left_eyeball).movePoint(2, 0, 16*r, 0);
		(*left_eyeball).movePoint(3, 0, 16*r, 0);
		//(*left_eyeball).movePoint(4, 0, 2*r, 0);
		//(*left_eyeball).movePoint(5, 0, 8*r, 0);
...
void doMenangis(FrameBuffer *FB, vector<Polygon*> *polies, Polygon *mouth, 
				Polygon *left_eye, Polygon *left_eyebrow, Polygon *right_eye, Polygon *right_eyebrow, Polygon *left_eyeball, Polygon *right_eyeball, int r) {
	// Menangis
	int f = 0;
	while ( f < 10 ) {
		(*FB).canvas();
		...
		(*left_eyeball).movePoint(2, 0, 8*r, 0);
		(*left_eyeball).movePoint(3, 0, 8*r, 0);

		(*right_eyeball).movePoint(2, 0, 8*r, 0);
		(*right_eyeball).movePoint(3, 0, 8*r, 0);
...
int main () {
	...
	Polygon left_eyeball;
	createEyeBall(&left_eyeball);
	left_eyeball.setPosition((FB.getVInfoX()/2)-180, (FB.getVInfoY()/2)-105, 0);
	//left_eyeball.setPosition(0, 0, 0);
	left_eyeball.setMultiplication(1.5);

	Polygon right_eyeball;
	createEyeBall(&right_eyeball);
	right_eyeball.setPosition((FB.getVInfoX()/2)+40, (FB.getVInfoY()/2)-105, 0);
	//left_eyeball.setPosition(0, 0, 0);
	right_eyeball.setMultiplication(1.5);
	...
	static Polygon* arrStar[] = {
		&face, &mouth, &left_eye, &left_eyebrow, &right_eye, &right_eyebrow, &nose, &right_ear, &left_ear, &left_eyeball, &right_eyeball
	};
	vector<Polygon*> star(arrStar, arrStar + sizeof(arrStar) / sizeof(arrStar[0]) );

	// static Polygon* arrStar[] = {
	// 	&face, &mouth, &left_eyeball
	// };
	// vector<Polygon*> star(arrStar, arrStar + sizeof(arrStar) / sizeof(arrStar[0]) );
	...
	doImut(&FB, &star, &mouth, &left_eye, &left_eyebrow, &left_eyeball, 1);
	sleep(2);
	doImut(&FB, &star, &mouth, &left_eye, &left_eyebrow, &left_eyeball, -1);

	doMenangis(&FB, &star, &mouth, &left_eye, &left_eyebrow, &right_eye, &right_eyebrow, &left_eyeball, &right_eyeball, 1);
	sleep(2);
	doMenangis(&FB, &star, &mouth, &left_eye, &left_eyebrow, &right_eye, &right_eyebrow, &left_eyeball, &right_eyeball, -1);
